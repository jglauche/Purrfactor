describe Purr18n do
  ...
  it 'scans devise edit.html.haml file' do
    ...
    expect(@loader.test_suggestions[2]).to eq "t('.leave_blank_if_you_dont_want_to_change_it')"
    expect(@loader.test_suggestions[3]).to eq "t('.characters_minimum')"

    expect(@loader.test_suggestions[4]).to eq "t('.we_need_your_current_password_to_confirm_your_changes')"

    expect(@loader.test_suggestions[5]).to eq "t('.cancel_my_account')"
